...
const StudentsBySub = () => {
  ...
  const students = useSelector(
    (state: RootState) => state.registeredStudents?.dataById
  );
  const id = useSelector((state: RootState) => state.registeredStudents?.subId);
    ...
    if (removeType === "single" && targetId) {
      ...
      try {
        const response = await axios.post(
          ...
          }api/v1/enterprise/students/remove-from-subscription/`,
          {
            subscription_id: id,
            enterprise_student_ids: [targetId],
          },
          {
            headers: {
      ...
      try {
        const response = await axios.post(
          ...
          {
            subscription_id: id,
            enterprise_student_ids: selectedStudents,
          },
          {
            headers: {
              Authorization: `Bearer ${token}`,
